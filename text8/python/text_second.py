...
def maybe_download(filename, path, expected_bytes):
    ...
    if not os.path.exists(path):
        filename, _ = urlretrieve(url=url + filename, filename=path)
...
filename = maybe_download('text8.zip', './data/text8.zip', 31344016)
...
def generate_batch(batch_size, num_skips, skip_window):
    ...
    #     Create buffer with data globally (each time this is run the buffer will diff)
