...
with graph.as_default():
    ...
    weights1 = tf.Variable(
        ...
        name="weights1")
    biases1 = tf.Variable(tf.zeros([num_nodes_1]), name="biases1")
    weights2 = tf.Variable(tf.truncated_normal([num_nodes_1, num_nodes_2], stddev=math.sqrt(2.0 / num_nodes_1)),
                           name="weights2")
    biases2 = tf.Variable(tf.zeros([num_nodes_2]), name="biases2")
    weights3 = tf.Variable(tf.truncated_normal([num_nodes_2, num_nodes_3], stddev=math.sqrt(2.0 / num_nodes_2)),
                           name="weights1")
    biases3 = tf.Variable(tf.zeros([num_nodes_3]), name="biases1")
    weights4 = tf.Variable(tf.truncated_normal([num_nodes_3, num_nodes_4], stddev=math.sqrt(2.0 / num_nodes_3)),
                           name="weights2")
    biases4 = tf.Variable(tf.zeros([num_nodes_4]), name="biases2")
    weights5 = tf.Variable(tf.truncated_normal([num_nodes_4, num_labels], stddev=math.sqrt(2.0 / num_nodes_4)),
                           name="weights1")
    biases5 = tf.Variable(tf.zeros([num_labels]), name="biases1")
    ...
    def get_logits_full(tf_dataset):
        ...
        logits_5 = tf.matmul(logits_4, weights5) + biases5
    ...
    regularizers = tf.nn.l2_loss(weights1) + tf.nn.l2_loss(weights2) + tf.nn.l2_loss(weights3) + tf.nn.l2_loss(
        weights4) + tf.nn.l2_loss(weights5)

    loss = tf.reduce_mean(loss + beta * regularizers)

    learning_rate = tf.train.exponential_decay(0.5, global_step=global_step, decay_steps=100000, decay_rate=0.96,
                                               staircase=True)
